...
@author: Quant Galore
"""
from feature_functions import binomial_option_price, bjerksund_stensland_greeks, Binarizer, round_proba
...
DATABASE_URL = f"postgresql://{os.getenv('DB_USER')}:{os.getenv('DB_PASSWORD')}@{os.getenv('DB_HOST')}:{5432}/{os.getenv('DB_NAME')}?sslmode=prefer"

initial_time = datetime.now()
calendar = get_calendar("NYSE")
underlying_ticker = "SPY"
end_date = datetime.now(tz=pytz.timezone("US/Eastern")).date()

# --- Auto-resume from last saved date ---
from sqlalchemy import inspect

engine = create_engine(DATABASE_URL)
inspector = inspect(engine)
if f"{underlying_ticker}_option_backtest" in inspector.get_table_names():
    try:
        with engine.connect() as conn:
            last_date_df = pd.read_sql(
                f'SELECT MAX("timestamp") AS last_date FROM {underlying_ticker}_option_backtest', con=conn
            )
        last_date = pd.to_datetime(last_date_df["last_date"].iloc[0]).date()
        start_date = (last_date + timedelta(days=1)).strftime("%Y-%m-%d")
        print(f"📅 Resuming from {start_date}")
    except Exception as e:
        print(f"⚠️ Could not detect last date, starting from 2022-11-01: {e}")
        start_date = "2022-11-01"
else:
    print("🆕 No existing dataset found, starting full build from 2022-11-01.")
    start_date = "2022-11-01"
...
for date in trade_dates:
    ...
    try:
        if date == trade_dates[-1]:
            ...
            previous_trading_date = trade_dates[np.where(trade_dates == date)[0][0] - 1]
            ...
            expiration_date = trade_dates[np.where(trade_dates == date)[0][0] + 1]
        ...
        if len(put_data_list) == len(call_data_list) and len(put_data_list) > 0 and len(call_data_list) > 0:
...
# --- Append new rows instead of dropping tables ---
Pre_Training_Dataset.to_sql(f"{underlying_ticker}_option_backtest", con=engine, if_exists='append', index=True)
Option_Chain_DataFrame = pd.concat(option_chain_list).reset_index()
Option_Chain_DataFrame.to_sql(f"{underlying_ticker}_option_chain", con=engine, if_exists='append', index=True)

final_time = datetime.now()
database_string = f"Database built on {end_date} in {final_time - initial_time}, last data point: {Pre_Training_Dataset.index[-1].strftime('%Y-%m-%d')}"
print(database_string)
send_message(message=database_string, subject=f"Database Operation on {end_date.strftime('%A')}, {end_date}")
